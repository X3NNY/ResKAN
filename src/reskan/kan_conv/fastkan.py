...
class FastKANLayer(nn.Module):
    ...
    def plot_curve(self, input_index: int, output_index: int, num_pts: int = 1000, num_extrapolate_bins: int = 2):
        ...
        x = torch.linspace(self.rbf.grid_min - num_extrapolate_bins * h, self.rbf.grid_max + num_extrapolate_bins * h, num_pts)
        with torch.no_grad():
    ...
    def regularization_loss(self, regularize_activation=1.0, regularize_entropy=1.0):
        ng = self.rbf.num_grids
        spline_weight = self.spline_linear.weight.view(self.output_dim, self.input_dim, ng)
        l1_fake = spline_weight.abs().mean(-1)
        reg_act = l1_fake.sum()
        p = l1_fake / (reg_act + 1e-10)
        reg_ent = -torch.sum(p * (p.log() + 1e-10))
...
class AttentionWithFastKANTransform(nn.Module):
    def __init__(self, q_dim: int, k_dim: int, v_dim: int, head_dim: int, num_heads: int, gating: bool = True):
        ...
        self.linear_g = FastKANLayer(q_dim, total_dim) if self.gating else None
        self.norm = head_dim**-0.5

    def forward(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, bias: Optional[torch.Tensor] = None) -> torch.Tensor:
